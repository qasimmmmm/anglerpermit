...
async function sendInviteEmail(args: {
  ...
  const from =
    ...
    "AnglerPermit <onboarding@resend.dev>";

  const text = [
    `Hi ${args.name},`,
    ``,
    `You've been invited to the AnglerPermit ops console.`,
    ``,
    `Login URL: ${loginUrl}`,
    `Email: ${args.to}`,
  ...
  const html = `
    <div style="font-family:ui-sans-serif,system-ui,sans-serif;max-width:520px;margin:0 auto;padding:24px;color:#0b1f2a">
      <h1 style="font-size:20px;margin:0 0 12px">You're invited to AnglerPermit Ops</h1>
  ...
  return deliver({
    from,
    to: args.to,
    subject: "Your AnglerPermit admin login",
    html,
    text,
    tag: "admin-invite",
  });
